...
def export_data(request):
    ...
    if request.method == "POST":
        ...
        safe_id = hashlib.sha256(request.user.email.encode()).hexdigest()[:16]
        response = JsonResponse(payload, json_dumps_params={"ensure_ascii": False, "indent": 2})
        response["Content-Disposition"] = f'attachment; filename="pogolab_export_{safe_id}.json"'
        return response
...
def delete_account(request):
    ...
    if request.method == "POST":
        ...
        with transaction.atomic():
            ...
            # 5. Borrar allauth EmailAddress y MFA Authenticator
            ...
            # 7. Anonimizar el usuario (preserva FK de AuditEvent)
