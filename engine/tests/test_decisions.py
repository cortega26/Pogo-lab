...
class TestEvaluate:
    ...
    def test_insufficient_sample(self):
        ...
        ctx = AnalysisContext(n=5, successes=0, p0=0.25, p_value=1.0,
                              effect_size=None, method_used="exact_binomial",
                              metric="hundo_rate", min_sample=30)
        recs = evaluate(ctx)
    ...
    def test_no_anomaly_conclusion(self):
        ...
        ctx = AnalysisContext(n=100, successes=1, p0=0.25, p_value=0.001,
                              effect_size=0.24, method_used="exact_binomial",
                              metric="hundo_rate", min_sample=30)
        recs = evaluate(ctx)
